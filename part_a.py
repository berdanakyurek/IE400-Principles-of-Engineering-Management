# Group 37
import gurobipy
from gurobipy import GRB

x_base = [20, 0, 30, 15, 0, 0, 35]
y_expected = [1,0,1,1,0,0,1]
# Patient specifications
total_dosage = 100
p = [1,1,0,0,1,1,0,0,0]
min_doses = [20,10,20,10,10,20,20]
max_doses = [80,50,100,100,70,90,50]

base_q_value = -5*p[0]-0.5*p[1]-12*p[2]-8*p[3]-5*p[4]-5*p[5]-p[6]-3*p[7]-2*p[8]
print(base_q_value)


while True:
    m = gurobipy.Model("model_a")

    y = []
    for i in range(7):
        y.append(m.addVar(vtype=GRB.BINARY, name="y" + str(i+1)))

    x = []
    for i in range(7):
        x.append(m.addVar(vtype=GRB.CONTINUOUS, name="x" + str(i+1)))

    for i in range(7):
        m.addConstr(x[i] >= 0, "c%d"%i)

    # Dosage constraint
    m.addConstr(sum(x) == total_dosage, "dosage_c")

    # Y constraints
    for i in range(7):
        m.addConstr(y[i] == y_expected[i])

    for i in range(7):
        m.addConstr((y[i] == 0) >> (x[i] == 0))

    #dosage constraints
    for i in range(7):
        m.addConstr((y[i] == 1) >> (x[i] >= min_doses[i]))
        m.addConstr((y[i] == 1) >> (x[i] <= max_doses[i]))

    q_score = base_q_value - 5 * y[0] - 6 * y[1] - 4 * y[2] - 4 * y[3] - 8 * y[4] - 6 * y[5] - 7 * y[6] + 0.28 * x[
        0] + 0.30 * x[1] + 0.25 * x[2] + 0.17 * x[3] + 0.31 * x[4] + 0.246 * x[5] + 0.4 * x[6]
    m.setObjective(q_score, GRB.MAXIMIZE)
    m.optimize()

    try:
        for i in range(len(x)):
            print("x[%d]="%i, x[i].X)
        for i in range(len(y)):
            print("y[%d]="%i, y[i].X)
        print(total_dosage)
        break
    except:
        total_dosage += 1
